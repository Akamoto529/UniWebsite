...
export default class RotatingGrid extends CanvasObject{
    constructor(canvas, pos, size) {
        super(canvas, pos, size);
        this.indent = 10;
        this.max_rotation_speed = 3;
    }
    handleClick(pos){
    ...
    set_elements(elems){
        this.elems = elems;
        this.rotationSpeeds = new Array(elems.length);
        for(let i = 0; i < this.rotationSpeeds.length; i++){
            this.rotationSpeeds[i] = Math.floor(Math.random() * (this.max_rotation_speed) + 1) * (Math.floor(Math.random() * 2) * 2 - 1);
        }
    ...
    draw() {
        ...
        for (let i = 0; i < this.row_cnt; i++) {
            for (let j = 0; j < Math.min(this.row_size, this.elems.length - i * this.row_size); j++){
                this.elems[i*this.row_size + j].rotation+=this.rotationSpeeds[i*this.row_size + j];
                this.elems[i*this.row_size + j].draw_in_a_box(this.elem_size);
            }
        }
        context.restore();
    }
